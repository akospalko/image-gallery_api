...
const getAllImageEntries = asyncWrapper(async (req, res) => {
  ...
  const imageEntries = await activeCollection.find({}).lean(); // get database entry as a plain object (.lean()) -> converts mongoose doc to plain obj -> allows to add properties to the obj (e.g. imageURL);
...
const getSingleImageEntry = asyncWrapper(async (req, res, next) => {
  ...
  const activeCollection = findActiveCollection(collection); // get active collection
  const imageEntry = await activeCollection.findOne({ _id: entryID }).lean(); // get db entry as plain object
...
const createImageEntry = asyncWrapper(async (req, res) => {
  ...
  const imageEntry = await activeCollection.create(entryData); 
...
const updateImageEntry = asyncWrapper(async (req, res, next) => {
  ...
  if(req.file) {
    const fetchedImageEntry = await activeCollection.findOne({ _id: entryID });
  ...
  const imageEntry = await activeCollection.findOneAndUpdate({ _id: entryID }, updateData, { // update entry
    new: true,
    runValidators: true
  });
