...
const resetUserPassword = asyncWrapper(async (req, res) => {
  ...
  if(!email) return res.status(404).json({successs: false, message: 'Provide email'}) // verify req body's content
  ...
  const link = `http://127.0.0.1:5173/password-forgot/${user._id}/${convertedToken}`; // front-end's route to create new password (/password-create)
